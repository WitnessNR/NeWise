...
def run_verified_robustness(file_name, n_samples, eps_0, p_n, q_n, 
                            method='NeWise', cnn_cert_model=False, 
                            activation = 'sigmoid', mnist=False, 
                            cifar=False, fashion_mnist=False, 
                            gtsrb=False):
    np.random.seed(1215)
    random.seed(1215)
    keras_model = load_model(file_name)
    ...
    for i in range(len(inputs)):
        ...
        LB_total, UB_total, _, _, _, _, _, _ = find_output_bounds(model.weights, model.biases, model.shapes, model.pads, model.strides, model.sizes, model.types, inputs[i].astype(np.float32), eps_0, p_n, activation, method)
        ...
        for j in range(i*9,i*9+9):
            ...
            LB, UB, A_u, A_l, B_u, B_l, pad, stride = find_output_bounds(weights, biases, shapes, model.pads, model.strides, model.sizes, model.types, inputs[i].astype(np.float32), eps_0, p_n, activation, method)
