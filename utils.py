...
def generate_data_myself(dataset, model, samples=10, start=0, cnn_cert_model=False, ids=None):
    if dataset == 'gtsrb':
...
def generate_data(dataset, samples, data_from_local=True, targeted=True, random_and_least_likely = False, skip_wrong_label = True, start=0, 
    ...
    if data_from_local:
        if dataset == 'mnist':
            json_path = 'data/mnist_100_infor.json'
            ...
            json_path = 'data/cifar10_100_infor.json'
            x_test, y_test = get_dataset_from_json(dataset, samples, json_path)
            if cnn_cert_model:
                x_test -= 0.5
        elif dataset == 'fashion_mnist':
            json_path = 'data/fashion_mnist_100_infor.json'
            x_test, y_test = get_dataset_from_json(dataset, samples, json_path)
